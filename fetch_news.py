"""
LeQuantum · 量子每天一口
自动抓取量子科技新闻 + Gemini AI翻译整理 + 生成带动画的静态网页
"""
...
5. 只返回JSON数组，不要任何其他文字

格式：
[{{"title_zh":"中文标题","title_en":"English title","summary_zh":"中文摘要100字以内","source":"来源","link":"链接","date":"日期","tags":["标签"],"importance":"high"}}]
...
{articles_text}"""
    try:
        ...
        print(f"  ⚠ AI处理失败: {e}")
    return [{"title_zh":a["title"],"title_en":a["title"],"summary_zh":a["summary"][:200],
    ...
    for a in articles[:5]:
        ...
        if src not in seen:
            ...
            company_html += f"""<div class="company-item">
              ...
              <div class="company-info"><div class="company-name">{src}</div><div class="company-update">{a.get('title_zh','')[:28]}...</div></div>
...
.brand-wrap{{margin-top:16px;display:flex;flex-direction:column;align-items:center;gap:5px;}}
.brand-name{{font-family:'Outfit',sans-serif;font-weight:900;font-size:36px;letter-spacing:-0.02em;}}
.brand-name .le{{color:rgba(255,255,255,0.35);}}
...
.brand-sub{{font-family:'Outfit',sans-serif;font-weight:300;font-size:11px;letter-spacing:0.4em;color:rgba(255,255,255,0.2);text-transform:uppercase;}}
.skip-btn{{position:fixed;bottom:30px;right:30px;font-family:var(--mono);font-size:10px;color:rgba(255,255,255,0.25);background:transparent;border:1px solid rgba(255,255,255,0.1);padding:6px 14px;border-radius:20px;cursor:pointer;letter-spacing:0.1em;transition:all 0.2s;z-index:1001;}}
...
@keyframes pulse{{0%,100%{{opacity:1;box-shadow:0 0 0 0 rgba(0,212,255,0.4);}}50%{{opacity:0.7;box-shadow:0 0 0 5px rgba(0,212,255,0);}}}}
.logo-mark{{font-family:'Outfit',sans-serif;font-weight:900;font-size:24px;letter-spacing:-0.02em;}}
.logo-mark .le{{color:rgba(255,255,255,0.38);}}
...
.logo-sub{{font-family:var(--mono);font-size:10px;color:var(--muted);letter-spacing:0.12em;}}
.header-right{{display:flex;align-items:center;gap:14px;}}
.date-badge{{font-family:var(--mono);font-size:11px;color:var(--muted);}}
.live-badge{{font-family:var(--mono);font-size:9px;color:#4ade80;background:rgba(74,222,128,0.08);border:1px solid rgba(74,222,128,0.18);padding:4px 10px;border-radius:4px;display:flex;align-items:center;gap:5px;}}
.live-dot{{width:5px;height:5px;background:#4ade80;border-radius:50%;animation:pulse 1.5s ease-in-out infinite;}}
.nav-bar{{display:flex;gap:26px;padding:13px 0;align-items:center;}}
...
.source-badge{{display:inline-flex;align-items:center;gap:4px;font-family:var(--mono);font-size:9px;color:#4ade80;background:rgba(74,222,128,0.05);border:1px solid rgba(74,222,128,0.12);padding:2px 8px;border-radius:3px;margin-bottom:10px;}}
...
.hero-visual{{height:150px;background:radial-gradient(ellipse at 30% 50%,rgba(123,79,255,0.2) 0%,transparent 60%),radial-gradient(ellipse at 80% 30%,rgba(0,212,255,0.15) 0%,transparent 50%),var(--deep);display:flex;align-items:center;justify-content:center;overflow:hidden;}}
.orbit-wrap{{width:120px;height:120px;position:relative;}}
.orbit{{position:absolute;border:1px solid rgba(0,212,255,0.2);border-radius:50%;top:50%;left:50%;transform:translate(-50%,-50%);}}
.o1{{width:46px;height:46px;animation:spin 4s linear infinite;}}
.o2{{width:80px;height:80px;animation:spin 7s linear infinite reverse;border-color:rgba(123,79,255,0.25);}}
.o3{{width:112px;height:112px;animation:spin 11s linear infinite;border-color:rgba(0,212,255,0.1);}}
@keyframes spin{{from{{transform:translate(-50%,-50%) rotate(0deg);}}to{{transform:translate(-50%,-50%) rotate(360deg);}}}}
.nucleus{{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:13px;height:13px;background:radial-gradient(circle,var(--accent),var(--accent2));border-radius:50%;box-shadow:0 0 16px rgba(0,212,255,0.7);}}
...
.article-title{{font-family:var(--serif);font-size:19px;font-weight:600;line-height:1.5;margin-bottom:8px;color:#fff;}}
.article-title-en{{font-family:var(--mono);font-size:11px;color:var(--muted);margin-bottom:11px;line-height:1.5;}}
.article-excerpt{{font-size:13px;line-height:1.85;color:rgba(232,234,242,0.68);margin-bottom:16px;}}
...
.analysis-num{{font-family:var(--mono);font-size:30px;font-weight:700;line-height:1;margin-bottom:8px;opacity:0.12;}}
...
.sidebar{{grid-column:2;display:flex;flex-direction:column;gap:16px;}}
.sidebar-card{{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:18px;}}
.sidebar-title{{font-family:var(--mono);font-size:9px;letter-spacing:0.2em;color:var(--accent);text-transform:uppercase;margin-bottom:13px;padding-bottom:10px;border-bottom:1px solid var(--border);}}
...
@media(max-width:900px){{
  ...
  .news-grid{{grid-template-columns:1fr;}}header{{padding:0 20px;}}
...
<!-- ── 动画层 ── -->
<div id="intro">
  <canvas id="anim-canvas" width="440" height="300"></canvas>
  <div class="brand-wrap">
    <div class="brand-name"><span class="le">le</span><span class="q">quantum</span></div>
    <div class="brand-sub">量子 · 每天一口</div>
  </div>
...
<main>
  <div>
    ...
    <div class="section-label" style="margin-top:4px;">更多动态 · MORE</div>
...
<footer>
  ...
  <div class="footer-links">
    <span class="footer-link">关于</span><span class="footer-link">来源说明</span><span class="footer-link">合作</span>
  </div>
</footer>
</div>
...
const QX=W/2-25, QY=H/2+5, QR=52;
...
function spawnBurst(x,y){{
  const colors=['#00d4ff','#7b4fff','#ff6fe8','#ffffff','#ffe566'];
  for(let i=0;i<26;i++){{
    const a=(Math.PI*2/26)*i+Math.random()*.4,s=2+Math.random()*5;
    particles.push({{x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:1,decay:.016+Math.random()*.014,r:2+Math.random()*3.5,color:colors[Math.floor(Math.random()*colors.length)]}});
  }}
}}
function spawnHearts(cx,cy){{
  for(let i=0;i<7;i++) heartPops.push({{x:cx+(Math.random()-.5)*80,y:cy-20,vy:-1.8-Math.random()*2,vx:(Math.random()-.5)*2.2,life:1,decay:.009,size:8+Math.random()*11,rot:Math.random()*.6-.3}});
}}
function spawnSparkles(cx,cy){{
  for(let i=0;i<12;i++){{
    const a=(Math.PI*2/12)*i;
    sparkles.push({{x:cx,y:cy,tx:cx+Math.cos(a)*(QR+50+Math.random()*18),ty:cy+Math.sin(a)*(QR+50+Math.random()*18),life:1,decay:.022,size:2.5+Math.random()*3}});
  }}
}}
...
function updatePhase(){{
  phaseT++;
  if(phase==='idle'){{
    mouthTarget=0.18+.06*Math.sin(frame*.035);
    if(phaseT>60) nextPhase('approach');
  }} else if(phase==='approach'){{
    const t=easeIO(clamp(phaseT/55,0,1));
    ballTarget=lerp(QX+155,QX+QR+18,t);
    mouthTarget=lerp(0.18,0.62,t);
    qSX=lerp(1,1.08,t); qSY=lerp(1,0.94,t);
    if(phaseT>=55) nextPhase('bite');
  }} else if(phase==='bite'){{
    const t=easeOut(clamp(phaseT/12,0,1));
    mouthTarget=lerp(0.62,0.02,t);
    if(phaseT===1){{
      qSX=1.28;qSVX=0;qSY=0.76;qSVY=0;qBVY=-5;
      spawnBurst(QX+QR+5,QY);
      shockwave={{x:QX,y:QY,r:QR,life:1}};
    }}
    ballScale=lerp(1,0,clamp(phaseT/10,0,1));
    ballAlpha=lerp(1,0,clamp(phaseT/8,0,1));
    if(phaseT>=14) nextPhase('chew');
  }} else if(phase==='chew'){{
    mouthTarget=0.12+.08*Math.abs(Math.sin(phaseT*.45));
    qSX=1+.05*Math.sin(phaseT*.5); qSY=1-.04*Math.sin(phaseT*.5);
    if(phaseT>35) nextPhase('happy');
  }} else if(phase==='happy'){{
    ...
    sceneFade=clamp(1-(phaseT/110),0,1);
    isHappy=false; mouthTarget=0.18+.04*Math.sin(frame*.04);
...
function updatePhysics(){{
  ...
  [qSX,qSVX]=spring(qSX,1,qSVX,0.15,0.65);
  [qSY,qSVY]=spring(qSY,1,qSVY,0.15,0.65);
  ...
  particles.forEach(p=>{{p.x+=p.vx;p.y+=p.vy;p.vx*=.91;p.vy*=.91;p.life-=p.decay;}});
  heartPops=heartPops.filter(p=>p.life>0);
  heartPops.forEach(p=>{{p.x+=p.vx;p.y+=p.vy;p.vy+=.025;p.life-=p.decay;}});
  sparkles=sparkles.filter(p=>p.life>0);
  sparkles.forEach(p=>{{p.x=lerp(p.x,p.tx,.07);p.y=lerp(p.y,p.ty,.07);p.life-=p.decay;}});
}}
...
function updateStars(){{
  stars.forEach(s=>{{
    ...
    if(s.isShooting){{
      s.shootTimer--;
      if(s.shootTimer<=0){{
        s.shootX=-10;s.shootY=Math.random()*H*.4;
        s.shootVX=4+Math.random()*6;s.shootVY=1+Math.random()*2.5;
        s.shootAlpha=1;s.tail=[];s.shootTimer=180+Math.random()*280;
      }}
      if(s.shootAlpha>0){{
        s.tail.unshift({{x:s.shootX,y:s.shootY}});
        if(s.tail.length>14)s.tail.pop();
        s.shootX+=s.shootVX;s.shootY+=s.shootVY;
        s.shootAlpha-=.028;
        if(s.shootX>W+50||s.shootAlpha<=0)s.shootAlpha=0;
      }}
    }}
  }});
...
function drawBG(sf){{
  ...
  if(!sf){{
    bgP.forEach(p=>{{
      ...
      ctx.fillStyle=`hsla(${{p.hue}},80%,70%,${{.1+.07*Math.sin(p.ph)}})`;ctx.fill();
    }});
  }}
  ctx.save();ctx.globalAlpha=sf?.01:.022;ctx.strokeStyle='#00d4ff';ctx.lineWidth=.5;
  const gs=40;
...
function drawStarfield(){{
  nebulae.forEach(n=>{{
    const g=ctx.createRadialGradient(n.x,n.y,0,n.x,n.y,n.r*(1+.15*Math.sin(n.phase)));
    g.addColorStop(0,`hsla(${{n.hue}},70%,60%,0.035)`);g.addColorStop(.5,`hsla(${{n.hue}},60%,50%,0.012)`);g.addColorStop(1,'rgba(0,0,0,0)');
    ctx.beginPath();ctx.arc(n.x,n.y,n.r*1.5,0,Math.PI*2);ctx.fillStyle=g;ctx.fill();
  }});
  stars.forEach(s=>{{
    ...
    if(s.baseR>1.5){{
      const len=s.r*6;ctx.save();ctx.globalAlpha=alpha*.45;
      ctx.strokeStyle=`hsl(${{s.hue}},80%,85%)`;ctx.lineWidth=.5;
    ...
    if(s.isShooting&&s.shootAlpha>0){{
      ctx.save();
      s.tail.forEach((t,i)=>{{
        ctx.beginPath();ctx.arc(t.x,t.y,1.4*(1-i/s.tail.length),0,Math.PI*2);
        ctx.fillStyle=`rgba(255,255,255,${{s.shootAlpha*(1-i/s.tail.length)*.7}})`;ctx.fill();
      }});
      ctx.beginPath();ctx.arc(s.shootX,s.shootY,2.2,0,Math.PI*2);
      ctx.fillStyle=`rgba(255,255,255,${{s.shootAlpha}})`;ctx.shadowColor='#fff';ctx.shadowBlur=7;ctx.fill();ctx.shadowBlur=0;
      ctx.restore();
    }}
  }});
}}
...
function drawOrbits(cx,cy){{
  orbits.forEach((o,i)=>{{
    ...
    ctx.strokeStyle=i===0?'rgba(0,212,255,0.18)':i===1?'rgba(123,79,255,0.15)':'rgba(0,212,255,0.1)';
    ctx.lineWidth=1;ctx.stroke();
    const ea=eAngle*(i%2===0?1:-1)*(1+i*.3);
    const ex=o.rx*Math.cos(ea),ey=o.ry*Math.sin(ea);
    for(let t=1;t<=5;t++){{
      const ta=ea-t*.15;
      ctx.beginPath();ctx.arc(o.rx*Math.cos(ta),o.ry*Math.sin(ta),o.er*(1-t/6),0,Math.PI*2);
      ctx.globalAlpha=.3*(1-t/7);ctx.fillStyle=o.ec;ctx.fill();
    }}
    ctx.globalAlpha=1;
    ctx.beginPath();ctx.arc(ex,ey,o.er,0,Math.PI*2);
    ctx.fillStyle=o.ec;ctx.shadowColor=o.ec;ctx.shadowBlur=10;ctx.fill();ctx.shadowBlur=0;
    ctx.restore();
  }});
}}
...
function drawQ(cx,cy,mA,happy,sc=1,al=1){{
  ...
  ctx.beginPath();
  if(mA>0.02){{ctx.moveTo(0,0);ctx.arc(0,0,QR,mA,Math.PI*2-mA);ctx.lineTo(0,0);}}
  else ctx.arc(0,0,QR,0,Math.PI*2);
  ...
  fr.addColorStop(0,'rgba(255,255,255,0.36)');fr.addColorStop(.4,'rgba(255,255,255,0.07)');fr.addColorStop(1,'rgba(255,255,255,0)');
  ...
  const rim=ctx.createLinearGradient(-QR,-QR,QR,QR);
  rim.addColorStop(0,'rgba(255,255,255,0.26)');rim.addColorStop(.5,'rgba(0,212,255,0.14)');rim.addColorStop(1,'rgba(0,0,0,0)');
  ...
  if(happy&&starEyes.length===2){{
    [[-eyeOX,eyeOY],[eyeOX,eyeOY]].forEach(([ex,ey],i)=>{{
      const se=starEyes[i];if(!se)return;
      ctx.save();ctx.translate(ex,ey);ctx.scale(se.scale,se.scale);ctx.rotate(se.angle);
      ctx.beginPath();
      for(let p=0;p<5;p++){{const a=(Math.PI*2/5)*p-Math.PI/2,ai=a+Math.PI/5;p===0?ctx.moveTo(Math.cos(a)*8,Math.sin(a)*8):ctx.lineTo(Math.cos(a)*8,Math.sin(a)*8);ctx.lineTo(Math.cos(ai)*3.5,Math.sin(ai)*3.5);}}
    ...
    [-eyeOX,eyeOX].forEach(ex=>{{
      ...
      const px=ex+mA*.7;
      ctx.beginPath();ctx.arc(px,eyeOY,4.2,0,Math.PI*2);ctx.fillStyle='#0a1a2e';ctx.fill();
...
function drawBall(){{
  ...
  const gw=ctx.createRadialGradient(0,0,0,0,0,26);
  gw.addColorStop(0,'rgba(123,79,255,0.5)');gw.addColorStop(.5,'rgba(255,111,232,0.2)');gw.addColorStop(1,'rgba(0,0,0,0)');
  ctx.beginPath();ctx.arc(0,0,26,0,Math.PI*2);ctx.fillStyle=gw;ctx.fill();
  const g=ctx.createRadialGradient(-5,-5,2,0,0,17);
  g.addColorStop(0,'#e0b8ff');g.addColorStop(.4,'#9b59ff');g.addColorStop(.8,'#5b1fd4');g.addColorStop(1,'#2a0080');
  ...
  const hl=ctx.createRadialGradient(-4,-5,1,-3,-4,9);
  hl.addColorStop(0,'rgba(255,255,255,0.42)');hl.addColorStop(1,'rgba(255,255,255,0)');
  ctx.beginPath();ctx.arc(0,0,17,0,Math.PI*2);ctx.fillStyle=hl;ctx.fill();
  ctx.fillStyle='rgba(255,255,255,0.92)';ctx.font='bold 14px Outfit,sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';
  ctx.shadowColor='rgba(255,255,255,0.5)';ctx.shadowBlur=5;ctx.fillText('Q',0,1);ctx.shadowBlur=0;
  ctx.restore();
}}
...
function drawFX(){{
  ...
  heartPops.forEach(p=>{{
    ctx.save();ctx.globalAlpha=p.life;ctx.translate(p.x,p.y);ctx.rotate(p.rot);ctx.scale(p.size/18,p.size/18);
    ctx.beginPath();ctx.moveTo(0,-5);ctx.bezierCurveTo(5,-10,11,-6,11,0);ctx.bezierCurveTo(11,6,0,13,0,17);ctx.bezierCurveTo(0,13,-11,6,-11,0);ctx.bezierCurveTo(-11,-6,-5,-10,0,-5);
    ctx.fillStyle='#ff6b9d';ctx.shadowColor='#ff6b9d';ctx.shadowBlur=9;ctx.fill();ctx.shadowBlur=0;ctx.restore();
  }});
  sparkles.forEach(p=>{{
    ctx.save();ctx.globalAlpha=p.life*.9;ctx.translate(p.x,p.y);
    ctx.beginPath();
    for(let i=0;i<4;i++){{const a=(Math.PI/2)*i,ai=a+Math.PI/4;i===0?ctx.moveTo(Math.cos(a)*p.size,Math.sin(a)*p.size):ctx.lineTo(Math.cos(a)*p.size,Math.sin(a)*p.size);ctx.lineTo(Math.cos(ai)*p.size*.3,Math.sin(ai)*p.size*.3);}}
    ctx.closePath();ctx.fillStyle='#ffe566';ctx.shadowColor='#ffe566';ctx.shadowBlur=7;ctx.fill();ctx.shadowBlur=0;ctx.restore();
  }});
  if(shockwave){{
    ctx.beginPath();ctx.arc(shockwave.x,shockwave.y+qBY,shockwave.r,0,Math.PI*2);
    ctx.strokeStyle=`rgba(0,212,255,${{shockwave.life*.55}})`;ctx.lineWidth=3*shockwave.life;ctx.stroke();
  }}
}}
...
function loop(){{
  frame++;
  updatePhase(); updatePhysics(); updateStars();
  ctx.clearRect(0,0,W,H);

  if(phase==='starfield'){{
    drawBG(true); drawStarfield();
    if(finalQAlpha>0.01){{
      ctx.save();ctx.globalAlpha=finalQAlpha*.7;
      drawOrbits(W/2,H/2);
      ctx.restore();
    ...
    drawBG(false);
    if(phase==='fadeout'){{
      ctx.save();ctx.globalAlpha=1-sceneFade;ctx.fillStyle='#030308';ctx.fillRect(0,0,W,H);ctx.restore();
      ctx.save();ctx.globalAlpha=sceneFade;
    }}
    const cy=QY+qBY;
    drawOrbits(QX,cy); drawFX(); drawBall();
    drawQ(QX,cy,mouth,isHappy); drawFX();
...
def main():
    ...
    if GEMINI_API_KEY:
        ...
        processed = [{"title_zh":a["title"],"title_en":a["title"],"summary_zh":a["summary"][:200],
                      "source":a["source"],"link":a["link"],"date":a["date"],
